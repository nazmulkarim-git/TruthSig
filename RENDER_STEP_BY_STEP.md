...
1) Create a new repo on GitHub (e.g., truthsig-phase2)
...
git init
git add .
git commit -m "TruthSig Phase 2 Render-ready"
git branch -M main
...
After deploy:
1) Open the Render Dashboard → truthsig-api
2) Copy its public URL (something like https://truthsig-api-xxxx.onrender.com)

Then:
3) Open Render Dashboard → truthsig-ui → Environment
4) Set:
   - TRUTHSIG_API_URL = (your copied API URL)
5) Click **Manual Deploy** (or it may auto redeploy)

## F) Test
1) Open your truthsig-ui public URL
2) Upload an image
...
- Upload limit is set to 25 MB via TRUTHSIG_MAX_MB in render.yaml (you can change it).