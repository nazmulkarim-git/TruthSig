...
DEFAULT_DB_PATH = os.getenv("TRUTHSIG_DB_PATH", "/tmp/truthsig.db")
...
def get_db_path() -> str:
    return os.getenv("TRUTHSIG_DB_PATH", DEFAULT_DB_PATH)
...
def init_db() -> None:
    ...
    if not _col_exists("cases", "user_id"):
        ...
        public = con.execute("SELECT id FROM users WHERE email = ?", ("public@truthsig",)).fetchone()
        if not public:
            public_id = _new_id("usr")
            con.execute(
                "INSERT INTO users (id, email, password_hash, created_at) VALUES (?, ?, ?, ?)",
                (public_id, "public@truthsig", "disabled", _utc_now_iso()),
            )
        public_id = con.execute("SELECT id FROM users WHERE email = ?", ("public@truthsig",)).fetchone()[0]
